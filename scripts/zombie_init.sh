...
# Optional software-rendering fallback (enable by setting ZOMBIE_SOFTWARE_RENDER=1)
...
ZOMBIE_WINE_DESKTOP_NAME="${ZOMBIE_WINE_DESKTOP_NAME:-EVE}"
ZOMBIE_WINE_DESKTOP_RES="${ZOMBIE_WINE_DESKTOP_RES:-1920x1080}"
...
if command -v xdpyinfo >/dev/null 2>&1 && xdpyinfo -display "$DISPLAY" >/dev/null 2>&1; then
    ...
    XVFB_SCREEN="${ZOMBIE_XVFB_SCREEN:-1920x1080x24}"
    Xvfb "$DISPLAY" -screen 0 "$XVFB_SCREEN" -nolisten tcp -ac +extension GLX +extension RENDER +extension RANDR +extension XFIXES &
    sleep 2
fi
